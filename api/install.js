...
const trimName = (n) => (n.length <= 100 ? n : n.slice(0, 97) + '...');
...
async function tryUpload(buf, origName) {
  ...
  if (create.object === 'error' || !create.upload_url) {
    console.warn('[Notion] file_upload create failed → fallback to external:', create);
    return null;
  }
  ...
  try {
    ...
    console.warn('[Notion] upload binary failed → fallback to external:', err.message);
    return null;
  }
}
  ...
  try {
    ...
    await Promise.all(assets.map(a => queue.add(async () => {
      ...
      if (buf.length <= 20 * 1024 * 1024) {
        const uploaded = await tryUpload(buf, a.name);
        if (uploaded) {
          newFiles.push({ name: uploaded.name, type:'file_upload', file_upload:{ id: uploaded.id } });
          return;
        }
        // else fall through to external link
      }
