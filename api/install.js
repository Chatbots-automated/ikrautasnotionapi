...
async function mondayAssets(id){
  ...
  const query=`query ($id:[ID!]){ items(ids:$id){ assets{ id name public_url file_size }}}`;
...
module.exports = async (req,res)=>{
  ...
  if(!itemId || !pageId) return res.status(400).json({ok:false,msg:'itemId or pageId missing'});
  ...
  try{
    ...
    await Promise.all(assets.map(a => queue.add(async () => {
      ...
      if(buf.length <= 20*1024*1024){
        ...
        if(up){
          children.push(blockForFile({type:'file_upload',file_upload:{id:up.id},name:up.name}));
          return;
        }
      }
      // fallback to external link
      console.log(`[External] ${a.name}`);
      children.push(blockForFile({type:'external',external:{url:a.public_url},name:trim(a.name)}));
    })));
...
/* build a Notion block (image or video) from a file object */
