...
/* ------------------------------------------------------------------ */
/* Very small cache to avoid re-sending the same block on warm runs   */
const SEEN = new Set();
...
module.exports = async (req, res) => {
  ...
  if (req.method !== 'POST')  return res.status(405).end('POST');
  ...
  try {
    ...
    const pageURL= page.url;                       // full https://www.notion.so/…
    if (!pageURL) throw new Error('page.url missing');

    /* 1️⃣  locate Monday item by Notion URL */
    const itemId = await findMondayItem(pageURL);
    if (!itemId) return res.status(200).end('no monday row');

    /* 2️⃣  pull every block under the page (100 is plenty for installers) */
    const blocks = await notion.blocks.children.list({ block_id: pageId, page_size: 100 });

    /* 3️⃣  filter NEW media blocks */
    const media = blocks.results.filter(b => {
      const t = b.type;
      if (!['image','video','file','pdf','audio'].includes(t)) return false;
...
/* ------------------------------------------------------------------ */
/* GraphQL helper – fetch monday item whose URL column matches value  */
...
/* ------------------------------------------------------------------ */
/* Upload ONE Notion block -> Monday “files” column                   */
async function uploadToMonday(itemId, block){
  /* grab a temporary download url from Notion */
  ...
  if (r.errors) throw new Error(r.errors[0].message);
  console.log(`[Monday] +${filename} → item ${itemId}`);
}
